...
10. Drugi **właściciel obciążenia** tworzy podsieć w sieci wirtualnej **produkcyjnej** , a następnie dodaje dwie maszyny wirtualne. Drugi **właściciel obciążenia** stosuje `environment` `managedBy` Tagi i do każdego zasobu.
...
5. Konto **właściciela obciążenia APP2** wysyła żądanie do **użytkownika operacji sieciowych** w celu komunikacji równorzędnej z usługą **APP2-prod-VNET** z **koncentratorem sieci wirtualnej**. Użytkownik **operacji sieciowych** tworzy połączenie komunikacji równorzędnej.
