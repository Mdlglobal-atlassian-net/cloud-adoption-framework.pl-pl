...
W przypadku małej migracji z udziałem samodzielnego zespołu ds. migracji jeden przebieg może obejmować wszystkie cztery fazy migracji dla jednego obciążenia (_oceny_, _migracji_, _optymalizacji_ oraz _zabezpieczania i zarządzania_). Zazwyczaj każdy z tych procesów jest współużytkowany przez wiele zespołów w różnych elementach roboczych w wielu przebiegach. W zależności od typu nakładu pracy, jego skali i ról przebiegi te mogą mieć różną formę.
